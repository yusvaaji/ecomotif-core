...
{
    public function payment(Request $request, RentalQuoteService $rentalQuoteService)
    {
        ...
        try {
            $breakdown = $rentalQuoteService->quote($car, $request->pickup_date, $request->return_date);
        } catch (\InvalidArgumentException $e) {
            return response()->json(['message' => $e->getMessage()], 422);
        }

        if ((int) $request->number_of_day !== (int) $breakdown['number_of_days']) {
            return response()->json([
                'message' => trans('translate.Invalid booking duration'),
            ], 422);
        }
